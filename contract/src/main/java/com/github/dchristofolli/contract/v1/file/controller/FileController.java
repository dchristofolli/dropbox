package com.github.dchristofolli.impl.file.controller;

import com.github.dchristofolli.impl.file.model.FileInput;
import com.github.dchristofolli.impl.file.service.FileService;
import com.github.dchristofolli.impl.user.mapper.UserMapper;
...
public class FileController {
    ...
    public void envioArquivo(@RequestParam MultipartFile arquivo,
        ...
        Optional<UserInput> user = userService.getUserById(idUsuario);
        fileService.sendFile(arquivo, user.get());
    }
    ...
    public ResponseEntity excluiArquivo(@PathVariable String idUsuario, String arquivo) throws IOException {
        Optional<UserInput> user = userService.getUserById(idUsuario);
        fileService.deleteFile(arquivo, user.get());
    ...
    public Page<FileInput> listaPaginada(@RequestParam(defaultValue = "1") int pagina,
        ...
        Optional<UserInput> userInput = userService.getUserById(usuario);
        return fileService.filesPagedList(quantidade, userInput.get());
    }
    ...
    public Page<FileInput> listaCompartilhadosComigo(@RequestParam(defaultValue = "1") int pagina,
        ...
        Optional<UserInput> userInput = userService.getUserById(usuario);
        return fileService.listsSharedWithMe(pagina, quantidade, userInput.get());
    }
    ...
    public void baixarArquivo(@RequestParam String id,
        ...
        Optional<UserInput> user = userService.getUserById(id);
        fileService.downloadFile(user.get(), arquivo);
    }
    ...
    public void baixarCompartilhadoComigo(@RequestParam String id,
                                          @RequestParam String arquivo) throws IOException {
        Optional<UserInput> user = userService.getUserById(id);
        fileService.downloadSharedWithMe(arquivo, user.get());
    }
}
